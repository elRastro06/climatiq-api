...
app.get("/co2", async (req, res) => {
  ...
  if (!from) {
    ...
    res
      .send({ error: "Bad request. Destination point not specified" })
      .status(400);
    return;
  }

  let response = await axios.get(
    DISTANCE_API_URI + "/geocoding?location=" + from
  );
  ...
  response = await axios.get(
    DISTANCE_API_URI + "/distance?from=" + coordFrom + "&to=" + coordTo
  );
  ...
  const co2 = {
    ...
    shipping_tax: response.data.co2e / 100,
    shipping_tax_unit: "€",
  };
